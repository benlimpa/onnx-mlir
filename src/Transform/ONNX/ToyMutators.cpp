...
mlir::LogicalResult DivToMulPattern::match(mlir::Operation *op) const {
  /*
   * check if the current operation can be mutated
   */
  // the operation must have two inputs and one output
...
void DivToMulPattern::rewrite(
  ...
  if (!rhs.getType()
           ...
           .getElementType()
           .isa<mlir::FloatType>())
    rhs = create.onnx.cast(rhs, mlir::TypeAttr::get(rewriter.getF32Type()));

  rewriter.replaceOpWithNewOp<mlir::ONNXDivOp>(op, lhs, rhs);
}
} // namespace toy_mutators